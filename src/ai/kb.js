...
export const KB={
  ...
  faqs:[
    ...
    // —— 新增：把命理转成现实行动 ——
    {id:'g3', q:'我现在最应该先做什么？', kw:['先做什么','下一步','行动','优先级','怎么办','如何开始'], intent:'综合', anchor:'focus',
      answer:(d)=>{
        const y=d.wx.ys||'用神';
        const topic=d.cDySS.includes('官')?'把责任和边界写清楚':d.cDySS.includes('财')?'先盘点现金流与可持续收入':d.cDySS.includes('食神')||d.cDySS.includes('伤官')?'把想法做成一个可交付的小成果':'把当前最重要的目标拆成一周内能完成的一步';
        return [
          `先别同时解决所有问题：从「${topic}」开始。`,
          `你的命盘当前更适合把${y}方向的优势用在具体场景，而不是等待一个确定信号。`,
          '如果现实信息与命盘提示冲突，优先相信可验证的信息。',
          '今天写下一个目标、一个阻碍和一个 30 分钟内能完成的动作，完成后再决定下一步。'
        ];
      }, related:['g1','t2']},
    {id:'g4', q:'命理和现实冲突时该听谁的？', kw:['准不准','相信','现实','冲突','命理靠谱吗','参考'], intent:'玄学', anchor:'reasoning',
      answer:()=>[
        '把命理当作自我观察和提问的工具，不当作事实证明或决定授权。',
        '现实中的合同、数据、身体检查、对方明确表达和专业意见，优先级都高于命盘象征。',
        '命理最多帮助你发现倾向、盲点和可以准备的地方，不能替你承担选择的后果。',
        '一个实用标准：如果结论让你更清楚、更有行动力，它有参考价值；如果让你恐惧、冲动或依赖，就先停下来核验。'
      ], related:['t4','g1']},
    {id:'g5', q:'怎么问命盘问题，得到的回答更有用？', kw:['怎么提问','如何问','问问题','提问方式','问命盘'], intent:'玄学', anchor:'bazi',
      answer:()=>[
        '问题越具体，回答越容易落地。把「我以后会怎样」改成「未来三个月，我该怎样准备这次转岗？」。',
        '最好补充你正在面对的事实、选项和截止时间，命盘只负责提供一个观察角度。',
        '一次只问一个核心问题，并追问「依据是什么」和「我今天能做什么」。',
        '不要把结果当成保证；用它帮助你整理信息、比较选项，再回到自己的判断。'
      ], related:['g3','t4']}
  ]
};
